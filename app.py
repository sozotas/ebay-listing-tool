...
<style>
    ...
    .rival-box { background: #fff8f8; border: 1.5px solid #f5c0c0; border-radius: 10px; padding: 1rem 1.2rem; margin-bottom: 1rem; }
    .spec-tag { display: inline-block; background: #f5f5f5; border: 1px solid #e0e0e0; border-radius: 99px; padding: 3px 10px; font-size: 0.8rem; margin: 2px; color: #555; }
</style>
...
# ---- Session State 初期化 ----
for k, v in {
    "rival_data": None, "specs": [], "images_b64": [],
    "auth_token": "", "app_id": "", "dev_id": "", "cert_id": "",
    "env": "production", "title": "", "category_id": "",
    "description": "", "cond_id": "1000", "rival_item_id": "",
}.items():
    if k not in st.session_state:
        st.session_state[k] = v

def ebay_request(endpoint, xml_str, call_name, app_id, dev_id, cert_id, site_id, timeout=15):
    """eBay Trading API へPOSTして結果テキストを返す"""
    body = xml_str.strip()
    headers = {
        ...
        "X-EBAY-API-APP-NAME": app_id.strip(),
        "X-EBAY-API-DEV-NAME": dev_id.strip(),
        "X-EBAY-API-CERT-NAME": cert_id.strip(),
    }
    resp = requests.post(endpoint, data=body.encode("utf-8"), headers=headers, timeout=timeout)
    return resp.text

# ---- サイドバー: 認証設定 ----
with st.sidebar:
    st.markdown("### 🔐 認証設定")
    env = st.selectbox("環境", ["production", "sandbox"],
                       format_func=lambda x: "🟢 本番 (Production)" if x == "production" else "🟡 サンドボックス (テスト)")
    ...
    cert_id = st.text_input("Cert ID", type="password",
                            ...
                            placeholder="PRD-XXXXXXXXXXXXXXXXXX")
    ...
    if st.button("📡 接続テスト", use_container_width=True):
        if not auth_token.strip():
            ...
            st.error("App ID を入力してください")
        else:
            endpoint = ("https://api.ebay.com/ws/api.dll"
                        if env == "production" else "https://api.sandbox.ebay.com/ws/api.dll")
            xml = (
                '<?xml version="1.0" encoding="utf-8"?>'
                '<GeteBayOfficialTimeRequest xmlns="urn:ebay:apis:eBLBaseComponents">'
                '<RequesterCredentials>'
                '<eBayAuthToken>' + auth_token.strip() + '</eBayAuthToken>'
                '</RequesterCredentials>'
                '</GeteBayOfficialTimeRequest>'
            )
            try:
                result = ebay_request(endpoint, xml, "GeteBayOfficialTime",
                                      app_id, dev_id, cert_id, site_id)
                if "Success" in result:
                    m = re.search(r"<Timestamp>(.*?)</Timestamp>", result)
                    ts = m.group(1) if m else ""
                    st.success(f"✅ 接続成功！ eBayサーバー時刻: {ts}")
                elif "HTML" in result or "Unavailable" in result:
                    st.error("接続失敗: eBayサーバーがHTMLを返しました。認証情報を確認してください。\n\n" + result[:300])
                else:
                    m = re.search(r"<LongMessage>(.*?)</LongMessage>", result)
                    err = m.group(1) if m else result[:300]
                    st.error(f"接続失敗: {err}")
...
with tab_item:
    ...
    with col_input:
        rival_id = st.text_input("ライバルセラーのItem ID",
                                 value=st.session_state["rival_item_id"],
                                 placeholder="例: 256789012345",
    ...
    if fetch_clicked and rival_id:
        if not app_id.strip():
            ...
            url = (
                f"https://open.api.ebay.com/shopping"
                f"?callname=GetSingleItem&responseencoding=JSON"
                f"&appid={app_id.strip()}&siteid=0&version=967"
                f"&ItemID={rival_id.strip()}"
                f"&IncludeSelector=ItemSpecifics,Description"
            )
            with st.spinner("eBay Shopping APIから取得中..."):
                try:
                    ...
                    if data.get("Ack") == "Failure":
                        ...
                        st.success("✅ アイテム情報を取得しました！下の項目を選んで適用してください。")
                    else:
    ...
    if st.session_state["rival_data"]:
        ...
        cond_label2 = cond_map.get(item.get("ConditionID",""), "—")
        ...
        with c2:
            ...
            st.markdown(f"`{price_cur} {price_val}` &nbsp; `カテゴリ: {cat_id} {cat_name}` &nbsp; `状態: {cond_label2}` &nbsp; `仕様: {len(specs_raw)}件`", unsafe_allow_html=True)
            if specs_raw:
                preview = " · ".join(
        ...
        if st.button("✅ フォームに適用する", type="primary"):
            ...
            if apply_title and item.get("Title"):
                st.session_state["title"] = item["Title"][:80]; applied.append("タイトル")
            if apply_cat and item.get("PrimaryCategoryID"):
                st.session_state["category_id"] = item["PrimaryCategoryID"]; applied.append("カテゴリID")
            if apply_desc and item.get("Description"):
                st.session_state["description"] = item["Description"]; applied.append("説明文")
            if apply_specs and specs_raw:
                st.session_state["specs"] = [
                    {"n": s["Name"], "v": s["Value"][0] if isinstance(s["Value"],list) else s["Value"]}
            ...
            if apply_cond and item.get("ConditionID"):
                st.session_state["cond_id"] = item["ConditionID"]; applied.append("状態")
            if apply_price and price_val != "—":
                st.session_state["rival_price"] = price_val; applied.append("価格")
    ...
    uploaded_files = st.file_uploader(
        ...
        type=["jpg","jpeg","png"], accept_multiple_files=True, key="img_uploader")
    if uploaded_files:
    ...
    st.markdown("---")
    st.markdown("#### 📋 基本情報")
    title = st.text_input("商品タイトル *", value=st.session_state["title"], max_chars=80,
                          placeholder="例: Apple iPhone 15 Pro 256GB Natural Titanium Unlocked")
    ...
    with col_cat:
        CATEGORIES = {
            "": "選択してください", "9355": "携帯電話 (9355)", "31388": "スマートフォン (31388)",
            "175673": "ゲーム機 (175673)", "139971": "カメラ (139971)",
            "11450": "衣類・ファッション (11450)", "888": "スポーツ (888)",
            "267": "本・雑誌 (267)", "11116": "おもちゃ (11116)", "58058": "家電 (58058)", "1": "その他 (1)",
        }
    ...
    st.markdown("#### ⭐ 商品状態")
    COND_OPTIONS = {"1000":"✨ 新品","1500":"📦 新品同様","2000":"👍 良好","2500":"🙂 普通","3000":"🔧 やや傷あり","7000":"⚠️ ジャンク"}
    ...
    description = st.text_area("説明文 *", value=st.session_state["description"], height=180,
                               placeholder="商品の詳細、スペック、付属品、注意事項などを記載してください。")
...
with tab_price:
    ...
    with col_price:
        default_price = float(st.session_state.get("rival_price",0.01)) if st.session_state.get("rival_price") else 0.01
        main_price = st.number_input("即決価格 *" if listing_type=="FixedPriceItem" else "予約価格",
    ...
    with col_ship:
        shipping_option = st.selectbox("配送オプション", [
            ...
            format_func=lambda x: {
                ...
                "USPSFirstClass":"USPS First Class","USPSPriority":"USPS Priority",
                "FedExGround":"FedEx Ground","UPSGround":"UPS Ground",
            }[x])
    with col_cost:
...
with tab_preview:
    st.markdown("#### 👁 出品プレビュー")
    cur_sym = {"USD":"$","JPY":"¥","EUR":"€","GBP":"£","AUD":"A$"}.get(currency,"$")
    price_disp = f"{cur_sym}{main_price:.2f}"
    ...
    with st.container(border=True):
        ...
        with pc1:
            if st.session_state["images_b64"]:
                st.image(base64.b64decode(st.session_state["images_b64"][0]), use_container_width=True)
            else:
                st.markdown("📷 画像なし")
        with pc2:
            st.markdown(f"### {st.session_state['title'] or '商品タイトルがここに表示されます'}")
            st.markdown(f"## :red[{price_disp}]")
            st.markdown(f"状態: **{cond_disp}** &nbsp;|&nbsp; 在庫: **{quantity}**", unsafe_allow_html=True)

    # XML生成関数
    def build_xml(full=False):
        img_xml = "\n".join(
            f"    <URL>data:image/jpeg;base64,{b64 if full else b64[:30]+'...'}</URL>"
        ...
        sp = f'<StartPrice currencyID="{currency}">{start_price:.2f}</StartPrice>' if listing_type=="Chinese" else ""
        ret = "ReturnsNotAccepted" if return_policy=="ReturnsNotAccepted" else "ReturnsAccepted"
        return f"""<?xml version="1.0" encoding="utf-8"?>
<AddItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  <RequesterCredentials><eBayAuthToken>{auth_token.strip() if full else '[YOUR_AUTH_TOKEN]'}</eBayAuthToken></RequesterCredentials>
  <Item>
    <Title>{st.session_state['title']}</Title>
    <Description><![CDATA[{st.session_state['description']}]]></Description>
    <PrimaryCategory><CategoryID>{st.session_state['category_id'] or '1'}</CategoryID></PrimaryCategory>
...
{img_xml}
    ...
    <ShippingDetails>
      ...
      <ShippingServiceOptions>
        ...
        <ShippingService>{shipping_option}</ShippingService>
        <ShippingServiceCost currencyID="{currency}">{shipping_cost:.2f}</ShippingServiceCost>
        <FreeShipping>{'true' if free_shipping else 'false'}</FreeShipping>
      </ShippingServiceOptions>
    </ShippingDetails>
    <ReturnPolicy>
      <ReturnsAcceptedOption>{ret}</ReturnsAcceptedOption>
...
{spec_xml}
  ...
  <WarningLevel>High</WarningLevel>
    ...
    with st.expander("📄 生成XML"):
        st.code(build_xml(full=False), language="xml")
        st.download_button("💾 XMLをダウンロード", data=build_xml(full=False),
                           file_name="ebay_request.xml", mime="text/xml")

    endpoint_url = ("https://api.ebay.com/ws/api.dll"
                    if env=="production" else "https://api.sandbox.ebay.com/ws/api.dll")
  ...
  --data @ebay_request.xml"""
    ...
    with col_info:
        st.info(f"環境: {'🟢 本番' if env=='production' else '🟡 サンドボックス'}")

    if submit:
        errors = []
        if not auth_token.strip():                  errors.append("Auth Token")
        if not st.session_state["title"].strip():   errors.append("商品タイトル")
        ...
        if errors:
            ...
            with st.spinner("出品リクエストを送信中..."):
                try:
                    xml_submit = build_xml(full=True)
                    result = ebay_request(endpoint_url, xml_submit, "AddItem",
                                         app_id, dev_id, cert_id, site_id, timeout=30)
                    with st.expander("📨 APIレスポンス", expanded=True):
                        st.code(result, language="xml")
                    if "Success" in result:
                        m = re.search(r"<ItemID>(\d+)</ItemID>", result)
                        item_id = m.group(1) if m else None
                        st.success(f"✅ 出品成功！ アイテムID: {item_id or '(レスポンス参照)'}")
                        if item_id:
                            base_url = "https://www.ebay.com" if env=="production" else "https://sandbox.ebay.com"
                            st.markdown(f"[eBayで確認する →]({base_url}/itm/{item_id})")
                    else:
                        m = re.search(r"<LongMessage>(.*?)</LongMessage>", result)
